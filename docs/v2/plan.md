...
| 里程碑 | 执行步骤 | 状态 | 核心交付物 |
|--------|----------|------|-----------|
| M1 | 1、2、5（配置 API） | ✅ 已完成 | `servers-config.json` 配置系统、类型定义、错误模型、多实例配置 API |
| M2 | 3、4、5（运行控制） | ✅ 已完成 | 单活运行时管理器、启停服务、运行控制 API（start/stop/runtime） |
| M3 | 6、7、8 | ✅ 已完成 | 终端日志流（SSE）、命令发送、命令补全 API（42 条命令词库） |
| M4 | 9、10、11、12 | ✅ 已完成 | 配置编辑实例上下文、前端 V2 交互、回归测试（85 个测试）、文档更新 |

**测试覆盖：** 总计 85 个测试全部通过
- 单元测试：`servers-config.test.ts`（6）、`terminal-buffer.test.ts`（5）、`admin-commands.test.ts`（8）、`runtime-manager.test.ts`（11）、`v2-regression.test.ts`（11）
- 集成/回归测试：`service.test.ts`（20）、`backup.test.ts`（6）、`lock.test.ts`（7）、`serializer.test.ts`（11）

**关键特性：**
- ✅ 多实例配置管理（`servers-config.json`）
- ✅ 自动从 `paths-config.json` 迁移
- ✅ 单活运行模式（同一时刻仅允许一个实例运行）
- ✅ 实时终端日志（SSE，2000 行环形缓冲）
- ✅ 多行命令批量发送
- ✅ 命令补全（Tab/方向键选择）
- ✅ 向后兼容的 `/api/config`（支持 `serverId` 参数）
...
- 状态：已完成。
- 步骤 9（现有配置编辑接入实例上下文）已落地：`/api/config` 支持 `serverId` query 参数，未传时默认使用首个实例，保持向后兼容；前端在切换实例后自动重载对应配置。
- 步骤 10（前端 V2 交互实现）已落地：新增「实例管理」标签页，含实例列表、控制面板（启动/停止按钮）、终端区（SSE 日志流、多行输入、命令补全）；单活交互约束（其他实例运行时禁用启动按钮）。
- 步骤 11（测试计划）已落地：新增 `tests/v2-regression.test.ts` 覆盖配置读写、校验、迁移逻辑；全量测试 85 个全部通过。
- 步骤 12（文档与发布准备）已落地：更新 `README.md` 含 V2 配置说明、迁移指南、API 参考；`servers-config.json.example` 提供完整示例。
